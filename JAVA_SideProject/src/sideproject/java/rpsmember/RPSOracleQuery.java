...
public interface RPSOracleQuery {
	...
	public static final String SQL_ITEM_BUY = "INSERT INTO " +TABLE_INVNAME+ " VALUES (INVENTORY_SEQ.NEXTVAL, ?, ?, ?, CURRENT_TIMESTAMP)";
	
	public static final String SQL_ITEM_COUNT = "SELECT "+COL_ITEM_COUNT+" FROM(SELECT * FROM "
												+TABLE_INVNAME+" ORDER BY "+COL_ITEM_DATE+" DESC) WHERE "
												+COL_MEMBER_ID+" = ? AND "+ COL_ITEM_NAME+" = ? AND ROWNUM = 1";
